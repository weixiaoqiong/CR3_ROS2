...
public:
    explicit CR5Commander(const std::string& ip, rclcpp::Node * node)
    ...
    void recvTask()
    {
        while (is_running_)
        {
            try
            {
                ...
                {
                    // std::cout << "tcp\n";
                    // RCLCPP_INFO(_node->get_logger(), "tcp");
                ...
                else
                {
                    try
                    {
                        RCLCPP_INFO(_node->get_logger(), "to connect...");
                        real_time_tcp_->connect();

                        RCLCPP_INFO(_node->get_logger(), "real_time_tcp_ connected");
                        dash_board_tcp_->connect();
                        RCLCPP_INFO(_node->get_logger(), "dash_board_tcp_ connected");
                    }
                    catch (const TcpClientException& err)
                    {
                        RCLCPP_INFO(_node->get_logger(), "tcp recv error"); // : %s", err.what());
                        usleep(3);
                    }
                }
            }
    ...
    void init()
    {
        ...
        {
            is_running_ = true;
            RCLCPP_INFO(_node->get_logger(), "Commander : init");
            thread_ = std::unique_ptr<std::thread>(new std::thread(&CR5Commander::recvTask, this));
        }
        catch (const TcpClientException& err)
        {
            RCLCPP_INFO(_node->get_logger(), "Commander : %s", err.what());
        }
    }
    ...
    void enableRobot()
    {
        ...
        char cmd1[100];
        sprintf(cmd1,"enable");
        RCLCPP_INFO(_node->get_logger(), "enable");
